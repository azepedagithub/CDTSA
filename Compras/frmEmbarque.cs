...
namespace CO
{
	public partial class frmEmbarque : DevExpress.XtraBars.Ribbon.RibbonForm
	{
		...
		private void UpdateControlsFromData(DataTable dt)
		{
			...
			{
				this.btnAplicar.Enabled = false;
				this.btnReversionContable.Enabled = true;
				this.btnEliminar.Enabled = false;
				this.btnEditar.Enabled = false;
			...
			{
				this.btnAplicar.Enabled = true;
				this.btnReversionContable.Enabled = false;
		...
		private void btnGenerarDocCPFactura_Click(object sender, EventArgs e)
		{
			try
			{
				...
				if (this.txtTotal.EditValue.ToString() == "" || this.txtTotal.EditValue == null)
					sMensajeError = sMensajeError + "	• Verifique los totales de la obligacion al proveedor \n\r";
				
				if (this.dtpFechaFactura.EditValue == null || this.dtpFechaFactura.EditValue.ToString() =="") 
					sMensajeError = sMensajeError + "	•  Seleccione la fecha de la factura \n\r";
				if (this.txtFactura.EditValue == null || this.txtFactura.EditValue.ToString() == "")
					sMensajeError = sMensajeError + "	•  Digite la factura \n\r";
				if (this.dtpFechaVence.EditValue == null || this.dtpFechaVence.EditValue.ToString() == "")
					sMensajeError = sMensajeError + "	•  Seleccione la Fecha de Vencimiento \n\r";
				if (sMensajeError != "") {
					MessageBox.Show("Por favor verifique los siguientes campos: \n\r " + sMensajeError, "Generar Documento de CP");
					return;
				}
				//GenerarDocumento CP
		...
		private void btnReversionContable_Click(object sender, EventArgs e)
		{
			try
			{
				if (this.linkAsientoMercaderia.Text != "" || this.linkAsientoMercaderia.Text != " --- --- ")
				{
					if (MessageBox.Show("Esta seguro que desea eliminar la contabilización de la factura del proveedor", "Revertir Aplicación Contable", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
					{
						String Asiento = this.linkAsientoMercaderia.Text.Trim();
						DataTable dtAsiento = CG.AsientoDAC.GetDataByAsiento(Asiento).Tables[0];
						if (dtAsiento != null && dtAsiento.Rows.Count > 0)
						{
							if (Convert.ToBoolean(dtAsiento.Rows[0]["Mayorizado"]))
							{
								//Generar Asiento de reversion y setear en nulo el asiento del embarque
								int IdEjercicio = (int)dtAsiento.Rows[0]["IDEjercicio"];
								String Periodo = dtAsiento.Rows[0]["Periodo"].ToString();
								String sAsientoAmulacion = "";

								sAsientoAmulacion = CG.AsientoDAC.Revertir(IdEjercicio, Periodo, Asiento, sUsuario);
							...
							{
								CG.AsientoDAC.InsertUpdateAsiento("D", "", dtAsiento.Rows[0]["Asiento"].ToString(), dtAsiento.Rows[0]["Tipo"].ToString());
							}
						}

						DAC.clsObligacionProveedorDAC.RemoveReferenciaContable(this.ID_Embarque, null);
					}
				}
			}
			catch (Exception ex) {
				MessageBox.Show("Ha ocurrido un error tratando de Revertir la aplicación contable de la factura del proveedor", "Revertir Aplicación Contable");
			}
		}
		...
		private void btnDesconfirmar_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
		{
			if (dtObligacionProveedor.Rows.Count > 0)
			{
				...
				if (drObl["Asiento"] != null && drObl["Asiento"].ToString() != "")
				{
					MessageBox.Show("Embarque", "No puede desconfirmar el embarque por que tiene  obligaciones asociadas al proveedor");
				}
				return;
			}
			if (this.Confirmada )
			{
				bool result = clsEmbarqueDAC.ConfirmarEmbarque((int)this.ID_Embarque, false, null);
				if (result)
				{
					MessageBox.Show("El Embarque ha sido des-confirmado");
					...
					this.tabFactura.PageVisible = false;
					this.xtraTabControl1.SelectedTabPage = this.tabDetalle;

					//TODO: eliminar Relacion de tabla de obligacion proveedor
					//CalcularTotalesEmbarque();
				}
			}
		}
