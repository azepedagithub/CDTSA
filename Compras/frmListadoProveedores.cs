...
namespace CO
{
    public partial class frmListadoProveedores : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        {
            if (!UsuarioDAC.PermiteAccion((int)Acciones.PrivilegiosComprasType.AgregarProveedores, _dtSecurity))
                this.btnAgregar.Enabled = false;
			if (!UsuarioDAC.PermiteAccion((int)Acciones.PrivilegiosComprasType.EditarProveedores, _dtSecurity))
                this.btnEditar.Enabled = false;
			if (!UsuarioDAC.PermiteAccion((int)Acciones.PrivilegiosComprasType.EliminarProveedores, _dtSecurity))
                this.btnEliminar.Enabled = false;
			if (!UsuarioDAC.PermiteAccion((int)Acciones.PrivilegiosComprasType.ExportarProveedores, _dtSecurity))
            ...
            if (_currentRow != null)
            {
                if (MessageBox.Show("Esta seguro que desea eliminar el Proveedor", "Listado de Proveedores", MessageBoxButtons.YesNo) == System.Windows.Forms.DialogResult.Yes)
                {
                    ...
                    try
                    {
                        ...
                        DAC.clsProveedorDAC.UpdateProveedor("D", Convert.ToInt64(_currentRow["IDProveedor"]), "", "", false, "", -1, -1, DateTime.Now, "", "", -1, -1, 0, 0, "", "", false, false, false, false, ConnectionManager.Tran);
