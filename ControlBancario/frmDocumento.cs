...
namespace ControlBancario
{
    public partial class frmDocumento : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        public frmDocumento()
        {
            ...
            _dsDocumento = MovimientosDAC.GetDataEmpty();
            _dtDocumento = _dsDocumento.Tables[0];
        ...
        public frmDocumento(int pIDCuentaBanco, int pIDTipo, int pIDSubTipo,int IdRuc, String pNumero)
        {
            InitializeComponent();
			this.Accion = "View";
        ...
        private void CargarDocumento(int IDCuentaBanco, int IDTipo, int IDSubTipo, int iDRuc,String Numero)
        {
            _dsDocumento = MovimientosDAC.GetData(IDCuentaBanco, IDTipo, IDSubTipo,iDRuc, Numero);
            _dtDocumento = _dsDocumento.Tables[0];
            _currentRow = _dsDocumento.Tables[0].Rows[0];
        ...
        private void InicializarNuevoElemento()
        {
            ...
            _currentRow = _dtDocumento.NewRow();
            ...
            try
            {
               ...
               // this.txtNumero.EditValue = CuentaBancariaDAC.NextConsecutivoCheque(Convert.ToInt32(this.slkupCuentaBancaria.EditValue));
				...
				else if (Accion == "Edit" || Accion == "View") 
                {
					...
					{
						HabilitarControles(false, false);
						this.slkupCuentaBancaria.ReadOnly = false;
					}
        ...
        private void btnEditar_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            if (Convert.ToBoolean(_currentRow["Anulado"]) == false)
            {
                Accion = "Edit";
                HabilitarControles(true, false);
                AplicarPrivilegios();
        ...
        private void GuardarAsiento()
        {
            ...
            if (Accion != "New")
            {
                if (_currentRow != null)
                {
                    ...
                    DataSet _dsChanged = _dsDocumento.GetChanges(DataRowState.Modified);
        ...
        private void slkupTipo_EditValueChanged(object sender, EventArgs e)
        {
            if (this.slkupTipo.EditValue != null && this.slkupTipo.EditValue.ToString() != "")
            {
				...
				Util.Util.ConfigLookupEdit(this.slkupSubTipo, SubTipoDocumentoDAC.GetData(iIDTipo, -1).Tables["Data"], "Descr", "IDSubtipo");
				Util.Util.ConfigLookupEditSetViewColumns(this.slkupSubTipo, "[{'ColumnCaption':'Tipo','ColumnField':'IDSubtipo','width':30},{'ColumnCaption':'Descripcion','ColumnField':'Descr','width':70}]");
        ...
        private void slkupCuentaBancaria_EditValueChanged(object sender, EventArgs e)
        {
            if (this.slkupCuentaBancaria.EditValue.ToString() != "" || this.slkupCuentaBancaria.EditValue != null)
            {
				...
				if (this.slkupTipo.EditValue != null ) {
					...
					if (Convert.ToInt32(this.slkupTipo.EditValue) == 1)
						this.txtNumero.EditValue = CuentaBancariaDAC.NextConsecutivoCheque(Convert.ToInt32(this.slkupCuentaBancaria.EditValue));
				}
            }
            else {
        ...
        private void CargarReloadData() {
            CargarDocumento(Convert.ToInt32(_currentRow["IDCuentaBanco"]),
                           Convert.ToInt32(_currentRow["IDTipo"]),
                           Convert.ToInt32(_currentRow["IDSubTipo"]),
